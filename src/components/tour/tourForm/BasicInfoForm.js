...
export default function BasicInfoForm(props) {
  ...
  } = useInput((value) => value >= 1, props.tour.duration);
  ...
  } = useInput((value) => value >= 2, props.tour.maxGroupSize);
  const {
    ...
    (value) => value.trim() === "" || (+value > 0 && +value <= 5),
    JSON.stringify(props.tour.ratingsAverage)
  );
  const {
  ...
  } = useInput((value) => +value >= 0, JSON.stringify(props.tour.price));
  ...
  } = useInput((value) => value.trim().length >= 100, props.tour.description);
  const {
  ...
  } = useInput((value) => value.trim().length >= 50, props.tour.summary);
  const {
    ...
    (value) =>
      ...
      value.trim() === "false" ||
      value.trim() === "",
    JSON.stringify(props.tour.secretTour)
  );
        ...
        <div className={style["form-control"]}>
          ...
          <select multiple onChange={handleSelectChange} value={selectedGuides}>
            {props.tourGuides.map((guide) => {
              return (
                <option value={guide._id} key={guide._id} defaultValue>
                  {guide.name}
                </option>
              );
            })}
